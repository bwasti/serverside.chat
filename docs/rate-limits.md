...
These are inner budgets. Existing outer bounds still apply: anonymous lobby posts also pass the 600-byte, per-minute, per-hour, global, concurrency, and fail-closed clanker moderation gate; WebSockets retain per-connection frame and origin checks; rooms retain connection, concurrency, hourly egress, database, source, scratch, file-size, log, realtime-publish, and hourly clanker-output quotas; the host process caps aggregate service executions and sockets; authentication endpoints retain their account-creation and OAuth-attempt ceilings. A site admin can tune room quotas from the room settings screen, but cannot relax host-wide Wasm, request-size, path-isolation, or process ceilings. Chunked and declared request bodies are cancelled at their byte ceilings rather than buffered without limit.
