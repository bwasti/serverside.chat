...
| Agent requests | 2 | 1 per 30s | 30s | 10m |
| SSH connection, auth, or session setup | 10 | 1 per 15s | 15s | 10m |
| Capability-shell or SFTP operations | 120 | 30 per second | 1s | 1m |

These are inner budgets. Existing outer bounds still apply: anonymous lobby posts also pass the 600-byte, per-minute, per-hour, global, concurrency, and fail-closed AI moderation gate; WebSockets retain a per-connection frame ceiling; rooms retain connection, concurrency, hourly egress, database, filesystem, file-size, and log quotas; authentication endpoints retain their account-creation and OAuth-attempt ceilings.
