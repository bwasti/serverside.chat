...
These are inner budgets. Existing outer bounds still apply: anonymous lobby posts also pass the 600-byte, per-minute, per-hour, global, concurrency, and fail-closed clanker moderation gate; WebSockets retain a per-connection frame ceiling; rooms retain connection, concurrency, hourly egress, database, filesystem, file-size, and log quotas; authentication endpoints retain their account-creation and OAuth-attempt ceilings.
