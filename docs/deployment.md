...
Caddy terminates public HTTP/TLS on ports 80 and 443 using [`deploy/Caddyfile`](../deploy/Caddyfile), automatically redirects HTTP to HTTPS, and proxies HTTP, WebSocket, and `/_dav/<room>/` WebDAV traffic to the application on `127.0.0.1:3000`. On-demand TLS is guarded by the application's loopback-only `/_internal/tls-allow` endpoint, which authorizes only native room hosts and owner-verified custom domains from an indexed local lookup. It never performs DNS or another network request during the TLS handshake. The TUI's SSH and SFTP protocols remain directly exposed on port 2222; they do not pass through the HTTP reverse proxy.
