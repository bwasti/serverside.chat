...
```dotenv
FIREWORKS_API_KEY=...
WEB_BASE_URL=http://your-host:3000
...
7. Verify the public page, the private-room 404 boundary, and SSH public-key login.
...
The initial endpoint is plain HTTP and WebSocket traffic on port 3000. Before treating the service as Internet-production, put it behind a hostname and TLS reverse proxy, wire authenticated browser sessions, configure a cloud firewall, and add backups for `/var/lib/serverside-chat`.
