...
The optional `authenticated` contribution policy deliberately broadens a visible room's write boundary to every signed-in canonical account, including source edits and clanker invocation. It does not grant membership, room administration, invitation authority, or canonical promotion. The settings UI labels it dangerous; public rooms using it should be treated as open collaborative workspaces whose shared output-token allowance can be consumed by any account.
...
## Clanker spend accounting

Every Fireworks request—room implementation turns, lobby guide turns, and anonymous lobby moderation—is charged to both a room and a process-global rolling hourly output-token budget. The host reserves the request's full configured maximum output in a dedicated SQLite ledger before network dispatch, inside the same immediate transaction that checks both ceilings. That prevents concurrent calls from racing past a limit. Provider-reported `completion_tokens` replaces the reservation after a response; if the provider times out, returns malformed data, or omits completion usage, the maximum-output reservation remains charged. Input and cached-input tokens are intentionally excluded from this product quota. A separate 512 KiB serialized-request ceiling bounds provider payload memory. A crashed in-flight request keeps its reservation for the full hour. Usage survives service restarts and follows a room rename, so restart and rename cannot reset spend.

Room and lobby clankers cannot publish raw model text. Every provider turn must return one schema-constrained function call; a dedicated finalization function carries `silent`, `answer`, or `blocked` plus a bounded plain-text message. The host recovers one complete, declared DeepSeek DSML call when a provider fails to lift it into the OpenAI-compatible `tool_calls` field, rejects malformed or undeclared markup, and never renders model reasoning or tool syntax. For concrete work, a bounded protocol-recovery pass rebuilds a compact context around the preserved working tree and asks the clanker to inspect, commit, and preview the existing edits; a malformed response after a confirmed commit is treated as successful silent completion. Persisted historical clanker messages containing leaked DSML are omitted when room state loads.

The production defaults are 1M output tokens per room per hour and 8M globally per hour; explicit positive integer environment values may lower or raise them. `OUT/H` in the room HUD includes completed and in-flight room output reservations. The global counter is deliberately host-side operational data rather than public tenant telemetry. The SQLite transaction coordinates multiple server processes only when they share the same data volume; horizontal replicas need a shared transactional quota service before they may independently contact the provider.

Token budgets limit volume, not exact money: models can have different prices per token, cached-token rules, and tool-call overhead. Before multiple price tiers or user-supplied models are offered, add a provider-billing-unit or currency budget as a separate bound. A collaborator can also exhaust a room's shared allowance without exceeding their request rate; per-account spend budgets and owner-configurable room allocations are the next fairness boundary.
