...
The production defaults are 250K tokens per room per hour and 2M globally per hour; explicit positive integer environment values may lower or raise them. `TOKENS/H` in the room HUD includes completed and in-flight room usage. The global counter is deliberately host-side operational data rather than public tenant telemetry. The SQLite transaction coordinates multiple server processes only when they share the same data volume; horizontal replicas need a shared transactional quota service before they may independently contact the provider.
