...
Every Fireworks request—room implementation turns, lobby guide turns, and anonymous lobby moderation—is charged to both a room and a process-global rolling hourly token budget. The host writes a conservative reservation to a dedicated SQLite ledger before network dispatch, inside the same immediate transaction that checks both ceilings. That prevents concurrent calls from racing past a limit. Provider-reported `total_tokens` replaces the estimate after a response; if the provider times out, returns malformed data, or omits usage, the larger conservative request-byte-plus-maximum-output estimate remains charged. A crashed in-flight request keeps its reservation for the full hour. Usage survives service restarts and follows a room rename, so restart and rename cannot reset spend.

The production defaults are 1M tokens per room per hour and 8M globally per hour; explicit positive integer environment values may lower or raise them. `TOKENS/H` in the room HUD includes completed and in-flight room usage. The global counter is deliberately host-side operational data rather than public tenant telemetry. The SQLite transaction coordinates multiple server processes only when they share the same data volume; horizontal replicas need a shared transactional quota service before they may independently contact the provider.
