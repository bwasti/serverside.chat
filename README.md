...
All provider calls—including lobby guidance and anonymous moderation—share a durable rolling token ledger. A request reserves a conservative input-plus-output estimate before dispatch, then replaces it with provider-reported total usage; timeouts and responses without usage remain charged at the estimate. The defaults are 250K tokens per room per hour and 2M tokens globally per hour. Configure them with `CLANKER_ROOM_TOKENS_PER_HOUR` and `CLANKER_GLOBAL_TOKENS_PER_HOUR`. The right HUD exposes each room's `TOKENS/H` meter.
