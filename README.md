...
- `/permissions contributions members|authenticated|admins|disabled`
...
All provider calls—including lobby guidance and anonymous moderation—share a durable rolling output-token ledger. A request reserves its configured maximum output before dispatch, then replaces that reservation with provider-reported completion usage; timeouts and responses without usage retain the maximum-output charge. Input tokens do not count toward this quota. The defaults are 1M output tokens per room per hour and 8M globally per hour. Configure them with `CLANKER_ROOM_TOKENS_PER_HOUR` and `CLANKER_GLOBAL_TOKENS_PER_HOUR`. The right HUD exposes each room's `OUT/H` meter.

Each clanker run has a 15-minute total budget, individual provider waits are capped at five minutes, and a run may use up to 256 model turns. The rolling output-token budget is the primary spend boundary, so it can stop a run before the turn ceiling. Fast transient network, rate-limit, and server failures retry within those bounds. The final two minutes are reserved by aborting an overlong earlier completion if necessary, then explicitly directing the model to recover partial writes, finish, commit, and create a preview. At four remaining turns it receives the same finalization instruction. Current provider activity appears concisely in the persistent chat status strip; terminal failures distinguish `provider timed out` from `15m run limit reached · work preserved` in the right-hand clanker-error log. A concrete implementation request cannot resolve as silence before a commit or one explicit blocker; the host forces one continuation and then surfaces an error if the model still stops without either.
