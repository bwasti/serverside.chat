export type SshEntryCommand = { kind: "account" } | { kind: "room"; roomName: string } | { kind: "shell"; roomName: string } | { kind: "invite"; token: string };
...
export function parseSshEntryCommand(value: string): SshEntryCommand {
  ...
  if (parts.length !== 2) throw new Error("usage: account | room <name> | shell <name> | invite <token>");
  ...
  throw new Error("usage: account | room <name> | shell <name> | invite <token>");
}
